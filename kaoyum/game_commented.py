...
class Game:
    def __init__(self, screen_size: tuple[int, int]):
        # กลุ่มวีชิมิชิมิ: สามารถสร้างเป็น sprite.Group() ได้แต่อาจต้องเขียนวิธีใหม่หมดเลยไม่จำเป็น
    ...
    def remove_dead_objects(self):
        ...
        # กลุ่มวีชิมิชิมิ: โค้ดซ้ำสามารถสร้างฟังก์ชั่นหรือ method ใหม่มาใช้ assign ได้
        # แต่จะมีหรือไม่มีก็ได้แหละ
        # หรือใช้ list comprehension
        # new_obstacles = [obs for obs in self.obstacles if obs.x >= -600]
        # new_color_changers = [cc for cc in self.color_changers if obs.x >= -600]
        # new_score_points = [sp for sp in self.score_points if obs.x >= -600]
