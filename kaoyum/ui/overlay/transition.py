...
class Transition(Widget):
    def __init__(self, screen_size: tuple[int, int], square_size: int):
        ...
        self.start_progress = Spring(0, natural_freq=14, damping_ratio=1.2) # First ripple
        self.end_progress = Spring(0, natural_freq=14, damping_ratio=1.2) # Second ripple
        self.wait_timer = 0
    ...
    def draw(self, screen: Surface, offset: tuple[int, int]):
        ...
        for x in range(w):
            for y in range(h):
                ...
                opacity1 = max(min(self.start_progress.value * length - (x + (h - y)), 1), 0)
                opacity2 = max(min(self.end_progress.value * length - (x + (h - y)), 1), 0)
